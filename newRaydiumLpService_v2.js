...
function parseCreateAmmLpParams(data) {
    ...
    const openTime = data.readBigUInt64LE(2);
    const initPcAmount = data.readBigUInt64LE(10);
    const initCoinAmount = data.readBigUInt64LE(18);
        ...
        for (const ix of instructions) {
            ...
            if (programId === RAYDIUM_AMM_PROGRAM_ID.toString() && ix.data.length > 0) {
                ...
                let tokenData = {
                    ...
                    K: BigInt(params.initPcAmount) * BigInt(params.initCoinAmount),
                    V: Math.min(BigInt(params.initPcAmount), BigInt(params.initCoinAmount)) / Math.max(BigInt(params.initPcAmount), BigInt(params.initCoinAmount))
                };
